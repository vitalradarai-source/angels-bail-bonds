...
// ── 1. REVERT FILTER — back to SpyFu sender only ─────────────────────────────
...
if (filterIdx !== -1) {
  ...
  console.log("✅ Filter reverted — SpyFu sender only (support@spyfu.com)");
}

// ── 2. REVERT PREPARE NODE — SpyFu-only prompts ───────────────────────────────
...
if (prepareIdx !== -1) {
  workflow.nodes[prepareIdx].parameters.jsCode = originalPrepareCode;
  console.log("✅ Prepare node reverted — SpyFu-only prompts restored");
}

// ── 3. REVERT FORMAT NODE — remove source variable, back to SpyFu task name ──
...
if (formatIdx !== -1) {
  ...
  // Remove the source line added by add-semrush-support.ts
  formatCode = formatCode.replace(
    /\n\s*const source = \$\('Prepare: Detect Type & Build Prompt'\)\.first\(\)\.json\.source \|\| 'SpyFu';\n/,
    "\n"
  );

  // Revert task name back to SpyFu prefix
  formatCode = formatCode.replace(
  ...
  console.log("✅ Format node reverted — task name back to SpyFu prefix");
}
...
if (putRes.ok) {
  console.log("\n✅ Workflow reverted — back to SpyFu-only state.");
} else {
